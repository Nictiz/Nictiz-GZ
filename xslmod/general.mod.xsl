...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:local="#local.btj_4bn_zhb" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" exclude-result-prefixes="#all">
    <!-- ================================================================== -->
    <!-- 
    ...
    <xsl:variable name="yatcs:default-dt-format" as="xs:string" select="'[Y]-[M01]-[D01] [H01]:[m01]:[s01]'">
        <!--~ Default date/time format string (`yyyy-mm-dd …`). -->
    </xsl:variable>
    <xsl:variable name="yatcs:default-dt-format-nl" as="xs:string" select="'[D01]-[M01]-[Y] [H01]:[m01]:[s01]'">
        <!--~ Date/time format string (Dutch: `dd-mm-yyyy …`). -->
    </xsl:variable>
    <xsl:variable name="yatcs:default-dt-format-en" as="xs:string" select="'[M01]-[D01]-[Y] [H01]:[m01]:[s01]'">
    ...
    <xsl:variable name="yatcs:language-nl" as="xs:string" select="'nl'">
        <!--~ Language code for Dutch -->
    </xsl:variable>
    <xsl:variable name="yatcs:language-en" as="xs:string" select="'en'">
    ...
    <xsl:variable name="yatcs:status-info" as="xs:string" select="'info'">
        <!--~ Generic info (a.k.a. OK) status/severity code. -->
    </xsl:variable>
    <xsl:variable name="yatcs:status-warning" as="xs:string" select="'warning'">
        <!--~ Generic warning status/severity code. -->
    </xsl:variable>
    <xsl:variable name="yatcs:status-error" as="xs:string" select="'error'">
        <!--~ Generic error status/severity code. -->
    </xsl:variable>
    <xsl:variable name="yatcs:status-debug" as="xs:string" select="'debug'">
        <!--~ Generic debug status/severity code. -->
    </xsl:variable>
    <xsl:variable name="yatcs:status-codes" as="xs:string+" select="($yatcs:status-info, $yatcs:status-warning, $yatcs:status-error, $yatcs:status-debug)">
    ...
    <xsl:variable name="yatcs:namespace-yatcs-common" as="xs:string" select="namespace-uri-for-prefix('yatcs', doc('')/*)">
        <!--~Namespace used for this code.  -->
    </xsl:variable>

    <xsl:variable name="yatcs:internal-error-prompt" as="xs:string" select="'Internal error: '">
    ...
    <xsl:function name="yatcs:char-repeat" as="xs:string">
    ...
    <xsl:function name="yatcs:q" as="xs:string">
    ...
    <xsl:function name="yatcs:capitalize" as="xs:string">
    ...
    <xsl:function name="yatcs:prefix-to-length" as="xs:string">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:variable name="prefix-string" as="xs:string" select="yatcs:char-repeat($prefix-char-to-use, $length - string-length($in))"/>
    ...
    <xsl:function name="yatcs:str2regexp" as="xs:string">
    ...
    <xsl:function name="yatcs:str2filename-safe" as="xs:string">
        ...
        <xsl:variable name="replace-char-to-use" as="xs:string" select="if (string-length($replace-char) lt 1) then '_' else substring($replace-char, 1, 1)"/>
    ...
    <xsl:function name="yatcs:item2element" as="element()?">
        <!--~ 
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="$error-on-non-resolve">
                        <xsl:call-template name="yatcs:raise-error">
                            <xsl:with-param name="msg-parts" select="($function-name-prompt, 'Document ', yatcs:q(string($item)), ' not found')"/>
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:sequence select="()"/>
    ...
    <xsl:function name="yatcs:str2bln" as="xs:boolean">
        <!--~ 
    ...
    <xsl:function name="yatcs:str2int" as="xs:integer">
        <!--~ 
    ...
    <xsl:function name="yatcs:str2seq" as="xs:string*">
    ...
    <xsl:function name="yatcs:str2id" as="xs:string">
        <!--~ 
    ...
    <xsl:function name="yatcs:att2str" as="xs:string">
    ...
    <xsl:function name="yatcs:elm2str" as="xs:string">
    ...
    <xsl:function name="yatcs:items2str" as="xs:string">
    ...
    <xsl:function name="yatcs:text2lines" as="xs:string*">
        <!--~
        ...
        <xsl:choose>
            ...
            then 0 
            else min(for $markdown-line in $textlines-2[normalize-space(.) ne ''] return yatcs:count-leading-whitespace($markdown-line))"/>
    ...
    <xsl:function name="yatcs:count-leading-whitespace" as="xs:integer">
    ...
    <xsl:template name="yatcs:raise-error">
        ...
        <xsl:param name="error-name" as="xs:string" required="no" select="$yatcs:status-error">
    ...
    <xsl:function name="yatcs:get-document-collection" as="document-node()*">
        ...
        <xsl:param name="hrefDir" as="xs:string"/>
        <xsl:param name="filenamePattern" as="xs:string"/>
        <xsl:param name="acceptEmpty" as="xs:boolean"/>

        <!-- Gets the collection of documents. Raises an error if this set is empty. -->
        <xsl:variable name="documents" as="document-node()*" select="collection(concat($hrefDir, '?select=', $filenamePattern))"/>
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:call-template name="yatcs:raise-error">
                    <xsl:with-param name="msg-parts" select="('No documents found in directory ', yatcs:q($hrefDir))"/>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
