...
{{DISPLAYTITLE:Beschikbaarstellen medicatiegegevens 9.1.0 - MA: addendum inhoudelijke gegevens kwalificatie}}
            ...
            <xsl:when test="$do-ada-files">
                ...
                <xsl:variable name="tabellen-overview-MGR" as="element()">
                    <tabellen>
                        ...
                        <xsl:for-each-group select="$ada-files/adaxml/data/*" group-by="substring-before(scenario-nr/@value, '.')">
                            <xsl:variable name="mainScenario" select="current-grouping-key()"/>
                            <tabel-simpel type="overview" subtype="MGR" title="Scenario's" main-scenario="{$mainScenario}">
                                <koprij>
                                    <kopkolom type="Scenario" waarde="Scenario"/>
                                    <kopkolom type="Beschrijving" waarde="Beschrijving"/>
                                    <!-- special handling for scenario 0 -->
...
== </xsl:text>
        ...
        <xsl:apply-templates select="$tableWithCommunity/tabel[@type = 'patient']" mode="wiki"/>
        ...
        <xsl:for-each-group select="$tableWithCommunity/tabel[not(@type = ('patient'))][@mbh-id]" group-by="concat('Scenario ', @scenario-nr, ' - ', @mbh-id)">
            <xsl:text>
==== </xsl:text>
        ...
        <xsl:for-each select="$tableWithCommunity/tabel[not(@type = ('patient'))][not(@mbh-id)]">
            <xsl:text>
==== </xsl:text>
            <xsl:value-of select="@title"/>
            <xsl:text> ====</xsl:text>
