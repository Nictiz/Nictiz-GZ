...
{{DISPLAYTITLE:Beschikbaarstellen medicatiegegevens 9.0.7 - </xsl:text>
        <xsl:value-of select="$abbreviation"/>
        <xsl:text>: addendum inhoudelijke gegevens </xsl:text>
...
==== </xsl:text>
    ...
    <xsl:function name="local:sub-scenario-ids" as="xs:string*">
        <xsl:param name="context" as="element()"/>
        <xsl:sequence select="$context/medicamenteuze_behandeling/*[local-name() eq $elementNameId]/identificatie/@value ! string(.)"/>
    </xsl:function>
