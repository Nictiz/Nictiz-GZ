...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:util="urn:hl7:utilities" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:local="urn:fhir:stu3:functions" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    <!-- ================================================================== -->
    <!--
        TBD
    -->
    ...
    <xsl:template match="/">
        <!-- Starts transformation for Medication Overview 9.0.7 from ADA to HL7 FHIR. -->
        <adaxml>
            <xsl:attribute name="xsi:noNamespaceSchemaLocation" select="'../ada_schemas/ada_beschikbaarstellen_medicatieoverzicht.xsd'"/>
            <meta status="new" created-by="generated" last-update-by="generated">
                <xsl:attribute name="creation-date" select="current-dateTime()"/>
                <xsl:attribute name="last-update-date" select="current-dateTime()"/>
            </meta>
            <data>
                <beschikbaarstellen_medicatieoverzicht app="mp-mp907" shortName="beschikbaarstellen_medicatieoverzicht" formName="uitwisselen_medicatieoverzicht" transactionRef="2.16.840.1.113883.2.4.3.11.60.20.77.4.102" transactionEffectiveDate="2016-03-23T16:32:43" prefix="mp-" language="nl-NL">
    ...
    <xsl:template match="f:List" mode="MedicationOverview-2.0">
        <!-- Transforms MedicationOverview f:List resource to ADA documentgegevens. -->
        <documentgegevens>
            <!-- document_datum -->
    ...
    <xsl:template match="f:date" mode="MedicationOverview-2.0">
        <!-- Transforms f:date to ADA document_datum. -->
        <document_datum>
    ...
    <xsl:template match="f:source" mode="MedicationOverview-2.0">
        <!-- Transforms f:source to ADA auteur with either auteur_is_zorgaanbieder or auteur_is_patient child. -->
    ...
    <xsl:template match="f:extension[@url = $MedicationOverview-Verification]" mode="MedicationOverview-2.0">
        <!-- Transforms MedicationOverview-Verification f:extension to ADA verificatie_patient and ADA verificatie_zorgverlener children. -->
        <verificatie_patient>
