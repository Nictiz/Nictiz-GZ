...
<xsl:stylesheet version="2.0" exclude-result-prefixes="#all" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:util="urn:hl7:utilities" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:local="#local.2023100511155144657630200">
    <!-- ================================================================== -->
    <!--
        TBD        
    -->
    ...
    <!-- This is a required parameter and matches the [base] of a FHIR server. 
        Expects *not* to end in / so we can make fullUrls like ${baseUrl}/Observation/[id] -->
    ...
    <xsl:variable name="DOCNAME" as="xs:string" select="base-uri(/)"/>
    ...
    <xsl:variable name="patients" as="element()*">
        <xsl:for-each-group select="//patient[not(patient)][ancestor::subject][not(@datatype = 'reference')][.//(@value | @code | @nullFlavor)]" group-by="string-join(for $att in nf:ada-pat-id(identificatienummer | patient_identificatie_nummer | patient_identification_number)/(@root, @value) return $att, '')">
            <xsl:for-each-group select="current-group()" group-by="nf:getGroupingKeyPatient(.)">
                ...
                <unieke-patient>
                    ...
                    <xsl:apply-templates select="current-group()[1]" mode="doPatientEntry-2.1">
                        ...
                        <xsl:with-param name="contact" select="($adaProcessedDocuments//bundle/contact[hcimroot/subject/patient/patient/patient_identification_number/@value = current-group()[1]/patient_identification_number/@value])" tunnel="yes" as="element()*"/>
                    </xsl:apply-templates>
                </unieke-patient>
            </xsl:for-each-group>
        </xsl:for-each-group>
    ...
    <xsl:template match="/">
        ...
        <!-- Start conversion. This conversion tries to account for all zibs in BgZ MSZ "beschikbaarstellen" in one go. 
            Either build a FHIR Bundle of type searchset per zib, or build individual files. -->
