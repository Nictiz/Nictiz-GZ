...
<xsl:stylesheet version="2.0" exclude-result-prefixes="#all" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:util="urn:hl7:utilities" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:uuid="http://www.uuid.org" xmlns:local="#local.2023103109525968639960100">
    <!-- ================================================================== -->
    <!--
        TBD
    -->
    ...
    <xsl:variable name="gstd-coderingen">
        <code rootoid="{$oidGStandaardGPK}" xmlns="http://hl7.org/fhir"/>
        <code rootoid="{$oidGStandaardHPK}" xmlns="http://hl7.org/fhir"/>
        <code rootoid="{$oidGStandaardPRK}" xmlns="http://hl7.org/fhir"/>
        <code rootoid="{$oidGStandaardZInummer}" xmlns="http://hl7.org/fhir"/>
    </xsl:variable>
        ...
        <xsl:for-each-group select="//product" group-by="nf:getProductGroupingKey(./product_code)">
            <xsl:for-each-group select="current-group()" group-by="nf:getGroupingKeyDefault(.)">
                ...
                <xsl:variable name="productCodeAsId" as="element()?">
                    <product_code value="{$most-specific-product-code/@code}" root="{$most-specific-product-code/@codeSystem}" xmlns="http://hl7.org/fhir"/>
                </xsl:variable>
    ...
    <xsl:variable name="bouwstenen-910" as="element(f:entry)*">
            ...
            <entry xmlns="http://hl7.org/fhir">
                ...
                <resource>
                    <xsl:call-template name="zib-AdministrationAgreement-3.0">
                        <xsl:with-param name="in" select="."/>
                        <xsl:with-param name="resource-id" select="                                 if ($referById) then                                     nf:removeSpecialCharacters(./identificatie/@value)                                 else                                     ()"/>
                    </xsl:call-template>
                </resource>
                <xsl:if test="string-length($searchMode) gt 0">
                    <search>
            ...
            <entry xmlns="http://hl7.org/fhir">
                ...
                <resource>
                    <xsl:call-template name="zib-Dispense-2.0">
                        ...
                        <xsl:with-param name="medicationdispense-id" select="                                 if ($referById) then                                     nf:removeSpecialCharacters(./identificatie/@value)                                 else                                     ()"/>
                    </xsl:call-template>
                </resource>
                <xsl:if test="string-length($searchMode) gt 0">
                    <search>
            ...
            <entry xmlns="http://hl7.org/fhir">
                ...
                <resource>
                    <xsl:call-template name="mp612dispensetofhirconversionadministrationagreement-1.0.0">
                        ...
                        <xsl:with-param name="medicationdispense-id" select="                                 if ($referById) then                                     (if (string-length(nf:removeSpecialCharacters(identificatie/@value)) gt 0) then                                         nf:removeSpecialCharacters(identificatie/@value)                                     else                                         (if (string-length(nf:removeSpecialCharacters(../identificatie/@value)) gt 0) then                                             nf:removeSpecialCharacters(../identificatie/@value)                                         else                                             uuid:get-uuid(.)))                                 else                                     ()"> </xsl:with-param>
                    </xsl:call-template>
                </resource>
                <xsl:if test="string-length($searchMode) gt 0">
                    <search>
    ...
    <xsl:template name="ext-zib-medication-medication-treatment-2.0">
        ...
        <extension url="http://nictiz.nl/fhir/StructureDefinition/zib-Medication-MedicationTreatment" xmlns="http://hl7.org/fhir">
            <valueIdentifier>
    ...
    <xsl:template name="organization-payload">
        ...
        <xsl:for-each select="./organisatie_naam[.//(@value | @code)]">
            <name value="{./@value}" xmlns="http://hl7.org/fhir"/>
        </xsl:for-each>
        <!-- contactgegevens -->
    ...
    <xsl:template name="patient-payload">
        ...
        <xsl:for-each select="geboortedatum[@value]">
            <birthDate value="{@value}" xmlns="http://hl7.org/fhir"/>
        </xsl:for-each>
        <!-- adresgegevens -->
    ...
    <xsl:template name="afleverlocatieReference" match="afleverlocatie" mode="doLocationReference">
        ...
        <reference value="{nf:getFullUrlOrId('LOCATION', nf:getGroupingKeyDefault(.), false())}" xmlns="http://hl7.org/fhir"/>
        <xsl:if test="./@value">
            <display value="{./@value}" xmlns="http://hl7.org/fhir"/>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="missing-type-reference-practitionerrole" match="zorgverlener" mode="doMissingTypeReferencePractitionerRole">
        ...
        <extension url="{$urlExtNLMissingTypeReference}" xmlns="http://hl7.org/fhir">
            <valueReference>
    ...
    <xsl:template name="reference-practitionerrole" match="zorgverlener" mode="doPractitionerRoleReference-907">
        ...
        <extension url="{$urlExtNLPractitionerRoleReference}" xmlns="http://hl7.org/fhir">
            <valueReference>
                <xsl:apply-templates select="." mode="doPractitionerRoleReference-2.0"/>
            </valueReference>
        </extension>
    ...
    <xsl:template name="TA-afspraakdatum" match="afspraakdatum | afspraak_datum_tijd" mode="TA-afspraakdatum">
        ...
        <extension url="http://nictiz.nl/fhir/StructureDefinition/zib-AdministrationAgreement-AuthoredOn" xmlns="http://hl7.org/fhir">
            <valueDateTime>
                <xsl:attribute name="value">
                    <xsl:call-template name="format2FHIRDate">
                        <xsl:with-param name="dateTime" select="@value"/>
    ...
    <xsl:template name="zib-DispenseRequest-2.2">
        ...
        <xsl:param name="searchMode" as="xs:string" select="'include'"/>

        <xsl:for-each select="$verstrekkingsverzoek">
            <xsl:call-template name="VV-in-MedicationRequest-2.2">
                <xsl:with-param name="verstrekkingsverzoek" select="."/>
                <xsl:with-param name="medicationrequest-id" select="                         if ($referById) then                             nf:removeSpecialCharacters(identificatie/@value)                         else                             ()"/>
            </xsl:call-template>
        </xsl:for-each>
    </xsl:template>
    ...
    <xsl:template name="medicatieoverzicht-9.0.7">
        ...
        <xsl:param name="searchMode" as="xs:string" select="'match'">
