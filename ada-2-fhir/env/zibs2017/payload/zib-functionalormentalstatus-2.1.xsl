...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://hl7.org/fhir" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:uuid="http://www.uuid.org" xmlns:local="urn:fhir:stu3:functions" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    ...
    <xsl:template name="zib-FunctionalOrMentalStatus-2.1" match="functionele_of_mentale_status[not(@datatype = 'reference')][.//(@value | @code | @nullFlavor)] | functional_or_mental_status[not(@datatype = 'reference')][.//(@value | @code | @nullFlavor)]" as="element(f:Observation)" mode="doZibFunctionalOrMentalStatus-2.1">
        <!-- Mapping of HCIM FunctionalOrMentalStatus concept in ADA to FHIR resource zib-FunctionalOrMentalStatus. -->
        <xsl:param name="in" select="." as="element()?">
            <!-- Node to consider in the creation of the Observation resource for FunctionalOrMentalStatus. -->
        </xsl:param>
