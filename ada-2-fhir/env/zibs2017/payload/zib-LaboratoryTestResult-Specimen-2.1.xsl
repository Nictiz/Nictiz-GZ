...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://hl7.org/fhir" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:uuid="http://www.uuid.org" xmlns:local="urn:fhir:stu3:functions" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    ...
    <xsl:template name="laboratoryResultSpecimenEntry" match="laboratory_test_result/specimen[.//(@value | @code | @nullFlavor)] | laboratorium_uitslag/monster[.//(@value | @code | @nullFlavor)]" mode="doLaboratoryResultSpecimenEntry-2.1" as="element(f:entry)">
        ...
        <xsl:param name="fhirResourceId">
            <!-- Optional. Value for the entry.resource.Specimen.id -->
    ...
    <xsl:template name="zib-LaboratoryTestResult-Specimen-2.1" match="//laboratorium_test[not(laboratorium_test)][not(@datatype = 'reference')][.//(@value | @code | @nullFlavor)] | //laboratory_test[not(laboratory_test)][not(@datatype = 'reference')][.//(@value | @code | @nullFlavor)]" as="element()" mode="doZibLaboratoryTestResultSpecimen-2.1">
        ...
        <xsl:param name="in" select="." as="element()?">
            <!-- Node to consider in the creation of the Specimen resource for LaboratoryTestResult. -->
        </xsl:param>
        ...
        <xsl:for-each select="$in">
            ...
            <xsl:variable name="collectionDetails" as="element()?">
                <xsl:if test="                         collection_period | verzamelperiode |                         collected_volume | verzamelvolume |                         collection_method | afname_procedure |                         anatomical_location | anatomische_locatie">
                    <collection>
