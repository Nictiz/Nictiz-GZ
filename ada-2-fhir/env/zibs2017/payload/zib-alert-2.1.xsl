                ...
                <Flag>
                    ...
                    <code>
                        ...
                        <xsl:choose>
                            ...
                            <!-- code is 1..1 in FHIR profile and its ValueSet (which includes NullFlavor OTH) has an extensible binding. When there is no AlertName, but there is a Problem, then add NullFlavor OTH with a textual pointer to Problem -->
                            <xsl:when test="conditie/probleem | condition/problem">
                                <xsl:call-template name="code-to-CodeableConcept">
                                    <xsl:with-param name="in" as="element()">
                                        <alert_naam code="OTH" codeSystem="{$oidHL7NullFlavor}" displayName="other" originalText="Zie probleem"/>
                                    </xsl:with-param>
