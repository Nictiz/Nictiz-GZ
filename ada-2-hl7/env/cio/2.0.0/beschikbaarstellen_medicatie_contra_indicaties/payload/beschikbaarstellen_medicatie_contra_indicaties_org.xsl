...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="urn:hl7-org:v3" xmlns:hl7="urn:hl7-org:v3" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:local="#local.2024011810474527324380100" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    <!-- ================================================================== -->
    <!--
        
            Created on: April 20, 2023
            Author: nictiz
            Mapping xslt for use case 'beschikbaarstellen medicatiecontraindicaties' in CiO. From ada to hl7.
    ...
    <xsl:template match="/" name="baseCiO">
        ...
        <xsl:param name="in" select="//beschikbaarstellen_medicatie_contra_indicaties" as="element()*">
        ...
        <xsl:choose>
            <xsl:when test="count($in) gt 1">
                <batch xmlns="">
                    <xsl:for-each select="$in">
                        <xsl:call-template name="medicatieContraIndicatie_20">
                            <xsl:with-param name="patient" select="./patient"/>
                            <xsl:with-param name="mci" select="./medicatie_contra_indicatie/alert"/>
                        </xsl:call-template>
                    </xsl:for-each>
                </batch>
            </xsl:when>
            <xsl:otherwise>
                <xsl:call-template name="medicatieContraIndicatie_20">
                    ...
                    <xsl:with-param name="mci" select="$in/medicatie_contra_indicatie/alert"/>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="medicatieContraIndicatie_20">
        ...
        <organizer classCode="CLUSTER" moodCode="EVN" xmlns:cda="urn:hl7-org:v3">
            ...
            <component typeCode="COMP">
                <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.26.10.14_20230419000000">
