...
<rule abstract="true"
   ...
   <assert role="error"
           ...
           see="https://art-decor.org/mediawiki/index.php?title=DTr1_CD">dtr1-18-CD: [RIM-002] 
<value-of select="$name"/> (@code=
<value-of select="$code"/> @codeSystem=
<value-of select="$codeSystem"/>) SHALL be distinct</assert>
   <let name="translations"
        value=".//hl7:translation/concat(@code,@codeSystem)"/>
   <assert test="count($translations)=count(distinct-values($translations))"
   ...
   <let name="RFC3066"
        value="'^[a-z]{2,3}(-[a-z]{2})?|[ix](-[a-z\d]{2,8})+$'"/>
   <let name="RFC3066restricted"
        value="'^[a-z]{2,3}(-[a-z]{2})?$'"/>
   <assert role="error"
