# This script returns the parameters as text as computed by the yatcs:get-combined-parameters() function
...
if [ "$1" = "-help" ]; then
  ...
  echo "Usage: `basename $0` [contains-string]"
  exit 0
fi
...
"$TOOLSBINDIR/saxon.sh" "-s:$BASEDIR/../data/dummy.xml" "-xsl:$BASEDIR/../xsl/get-parameters.xsl" "staticBaseUri=$PWD" "nameContains=$1"
exit 0