...
<xsl:stylesheet version="3.0" exclude-result-prefixes="#all" expand-text="false" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:narf="http://www.nictiz.nl/adarefs-functions" xmlns:array="http://www.w3.org/2005/xpath-functions/array" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:map="http://www.w3.org/2005/xpath-functions/map" xmlns:local="#local.2023032108485627665960100" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    ...
    <xsl:import href="../adarefs2ada-mp-930.xsl"/>

    <xsl:param name="ada-input" as="document-node()" required="false" select="/">
        <!-- The document to process, defined as a parameter, so we can (re-)define/override it in imported stylesheets. --> </xsl:param>


    <!-- ================================================================== -->
    <!-- PARAMETERS: -->

    <xsl:import href="../../../../xslmod/adarefs2ada-build-standard-parameters.mod.xsl"/>
    ...
    <xsl:variable name="inputFulladaFiles" as="document-node()*" select="yatcs:get-ada-files()">
        <!-- the input ada instances which contain the full patient/ma/vv which are referenced in $ada-input --> </xsl:variable>

    <xsl:param name="versionDir">9.3.0</xsl:param>
    <xsl:param name="transactionRef" as="xs:string">2.16.840.1.113883.2.4.3.11.60.20.77.4.395</xsl:param>
    ...
    <xsl:template name="ResolveContent">
        ...
        <xsl:param name="in" select=".">
            <!-- The input ada references transaction --> </xsl:param>
        ...
        <xsl:for-each select="$in">
            ...
            <xsl:if test="medicatieafspraak | verstrekkingsverzoek">
                ...
                <bouwstenen>
                    ...
                    <!-- only unique instances and in correct order -->
                    ...
                    <xsl:for-each select="$distinctMbhReferenceValues">
                        <xsl:variable name="currentZorgverlener" select="($inputFulladaFiles/adaxml/data/*/bouwstenen/zorgverlener[@id = current()])[1]"/>
                        <xsl:choose>
                            <xsl:when test="$mbhs/medicamenteuze_behandeling//voorschrijver/zorgverlener[@value = current()]">
                                <!-- this zorgverlener is a voorschrijver, let's override it -->
                                <xsl:apply-templates select="$currentZorgverlener" mode="copy-for-override"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:apply-templates select="$currentZorgverlener" mode="#current"/>
                            </xsl:otherwise>
                        </xsl:choose>
                     </xsl:for-each>
                    <xsl:for-each select="$distinctMbhReferenceValues">
