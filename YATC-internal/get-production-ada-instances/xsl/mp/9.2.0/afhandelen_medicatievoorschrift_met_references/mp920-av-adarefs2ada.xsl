...
<xsl:stylesheet version="3.0" exclude-result-prefixes="#all" expand-text="false" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:hl7="urn:hl7-org:v3" xmlns:narf="http://www.nictiz.nl/adarefs-functions" xmlns:array="http://www.w3.org/2005/xpath-functions/array" xmlns:nf="http://www.nictiz.nl/functions" xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:map="http://www.w3.org/2005/xpath-functions/map" xmlns:local="#local.202303210848557938030100" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    ...
    <xsl:param name="transactionRef" as="xs:string">2.16.840.1.113883.2.4.3.11.60.20.77.4.334</xsl:param>
    ...
    <xsl:template match="adaxml" mode="copy-for-resolve">
        ...
        <xsl:copy>
            <xsl:attribute name="xsi:noNamespaceSchemaLocation">../../../<xsl:value-of select="$versionDir"/>/sturen_afhandeling_medicatievoorschrift/ada_schemas/ada_sturen_afhandeling_medicatievoorschrift.xsd</xsl:attribute>
    ...
    <xsl:template match="opvragen_potentiele_contraindicaties" mode="copy-for-resolve">
        ...
        <sturen_afhandeling_medicatievoorschrift app="{@app}" shortName="sturen_afhandeling_medicatievoorschrift" formName="afhandelen_medicatievoorschrift" transactionRef="{$transactionRef}" transactionEffectiveDate="{$transactionEffectiveDate}" versionDate="" prefix="mp-" language="nl-NL" title="{@title}" desc="{@desc}" id="{narf:handle-ada-id(concat($concat2id,@id))}">
