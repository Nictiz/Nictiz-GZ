...
<xsl:stylesheet version="3.0" exclude-result-prefixes="#all" expand-text="true" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:hl7="urn:hl7-org:v3" xmlns:array="http://www.w3.org/2005/xpath-functions/array" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:map="http://www.w3.org/2005/xpath-functions/map" xmlns:local="#local.2023020714593486048760100" xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    <xsl:variable name="transaction" select="dataset[exists(@transactionId)]/@shortName"/>

    <!-- ================================================================== -->

    <xsl:template match="/">
        <!-- Root node start template -->
        <xsl:result-document href="{$outDir}{$transaction}-{$currentDatasetversion4FileName}_2_{$otherDatasetversion4FileName}.xml">
            ...
            <xsl:choose>
                <xsl:when test="$currentOtherDatasetConcept[@id != current()/@id]">
                    ...
                    <xsl:for-each select="condition">
                        <xsl:apply-templates select="." mode="content-compare-attrib">
                            <xsl:with-param name="currentOtherDatasetElement" select="$currentOtherDatasetConcept/condition"/>
                            <xsl:with-param name="attribName" select="('minimumMultiplicity', 'maximumMultiplicity', 'conformance', 'isMandatory')"/>
                        </xsl:apply-templates>
                    </xsl:for-each>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="example | name | desc | condition" mode="dataset-compare">
