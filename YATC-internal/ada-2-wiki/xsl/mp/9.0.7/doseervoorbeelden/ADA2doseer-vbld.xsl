...
<xsl:stylesheet version="3.0" exclude-result-prefixes="#all" expand-text="false" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:hl7="urn:hl7-org:v3" xmlns:file="http://expath.org/ns/file" xmlns:array="http://www.w3.org/2005/xpath-functions/array" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:map="http://www.w3.org/2005/xpath-functions/map" xmlns:local="#local.202303070821189775740100">
    ...
    <xsl:import href="https://raw.githubusercontent.com/Nictiz/HL7-mappings/master/ada_2_hl7/mp/2_hl7_mp_include_90.xsl"/>
    <xsl:import href="https://raw.githubusercontent.com/Nictiz/HL7-mappings/master/ada_2_fhir/zibs2017/payload/package-1.3.10.xsl"/>
    ...
    <xsl:param name="dir-4-ada" select="'C:\SVN\art_decor\trunk\ada-data\ada_2_wiki\mp\9.0.7\doseervoorbeelden\ada_instance\'"/>
...
{{DISPLAYTITLE:Medicatieproces 9.0.7: Voorbeelden doseringen}}
...
== </xsl:text><xsl:value-of select="$headingString"/> == <xsl:variable name="ada-files" select="collection(concat('ada_instance/', ., '?select=*.xml'))"/>
    ...
    <xsl:template match="adaxml/data/*" mode="wiki">
        <!-- Override of the template from the imported stylesheet. Makes it a suitable for doseervoorbeelden 9.0.7 -->
...
{{Collapse top|XML contents}}
        ...
        <syntaxhighlight lang="xml">
            <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9235_20181204143321">
                <xsl:with-param name="ma" select=".//medicatieafspraak"/>
            </xsl:call-template>
        </syntaxhighlight>
        <xsl:text>
