...
<xsl:stylesheet version="3.0" exclude-result-prefixes="#all" expand-text="false" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:array="http://www.w3.org/2005/xpath-functions/array" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:map="http://www.w3.org/2005/xpath-functions/map" xmlns:local="#local.2023030708211985943930100">
    ...
    <xsl:param name="transactionId">2.16.840.1.113883.2.4.3.11.60.20.77.4.286</xsl:param>
...
{{DISPLAYTITLE:Sturen medicatiegebruik 9.2.0: addendum inhoudelijke gegevens kwalificatie}}

=Addenda voor inhoudelijke gegevens=</xsl:text>
        <xsl:choose>
            <xsl:when test="$do-ada-files">
                <xsl:variable name="tabellen-overview-MGBS" as="element()">
                    <tabellen>
                        ...
                        <xsl:for-each-group select="$ada-files/adaxml/data/*" group-by="substring-before(scenario-nr/@value, '.')">
                            <tabel-simpel type="overview" subtype="MGBS" title="Scenario's" main-scenario="{current-grouping-key()}">
                                <koprij>
                                    ...
                                    <kopkolom type="MGB id" waarde="MGB id"/>
                                </koprij>
                                ...
                                <xsl:for-each-group select="current-group()" group-by="scenario-nr/@value">
                                    ...
                                    <rij>
                                        ...
                                        <kolom type="MGB id" waarde="{string-join(medicamenteuze_behandeling/(medicatiegebruik|medicatie_gebruik)/identificatie/@value, '&lt;br&gt;')}"/>
                                    </rij>
                                </xsl:for-each-group>
                            </tabel-simpel>
                        </xsl:for-each-group>
                ...
                <xsl:variable name="tabellen-overview-MGBO" as="element()">
                    <tabellen>
                        <!-- group by main scenario number, i.e. group all scenario's with scenario-nr '1.' together, also for 2. -->
                        <xsl:for-each-group select="$ada-files/adaxml/data/*" group-by="substring-before(scenario-nr/@value, '.')">
                            <tabel-simpel type="overview" subtype="MGBO" title="Scenario's" main-scenario="{current-grouping-key()}">
                                <koprij>
                                    <kopkolom type="Scenario" waarde="Scenario"/>
                                    <kopkolom type="Beschrijving" waarde="Beschrijving"/>
                                    <kopkolom type="MBH id" waarde="MBH id"/>
                ...
                <xsl:call-template name="yatcs:write-debug-document">
                    <xsl:with-param name="contents" select="$tabellen-overview-MGBS"/>
                    <xsl:with-param name="filename" select="'overview-mgbs'"/>
                    <xsl:with-param name="enabled" select="$debug"/>
                </xsl:call-template>
                <xsl:call-template name="yatcs:write-debug-document">
                    <xsl:with-param name="contents" select="$tabellen-overview-MGBO"/>
                    <xsl:with-param name="filename" select="'overview-mgbo'"/>
                    <xsl:with-param name="enabled" select="$debug"/>
                </xsl:call-template>
...
== Overzicht van scenario's voor sturend/beschikbaarstellend systeem ==
                ...
                <xsl:apply-templates select="$tabellen-overview-MGBS/tabel-simpel" mode="wiki">
...
== Overzicht van scenario's voor ontvangend systeem ==
                ...
                <xsl:apply-templates select="$tabellen-overview-MGBO/tabel-simpel" mode="wiki">
                    <xsl:sort select="                             if (replace(@main-scenario, '(\d+).*', '$1') castable as xs:integer) then                                 xs:integer(replace(@main-scenario, '(\d+).*', '$1'))                             else                                 0"/>
                    <xsl:sort select="@main-scenario"/>
                </xsl:apply-templates>
