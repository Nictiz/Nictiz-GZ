...
<!--
            ...
            Purpose: This XSL was created to facilitate mapping from ADA xml to wiki for example dosage information.
                ...
                History:
                    ...
                    2020-01-24 version 0.1 Initial version
...
<xsl:import href="https://raw.githubusercontent.com/Nictiz/HL7-mappings/master/ada_2_hl7/mp/9.1.0/2_hl7_mp_include_91.xsl"/>
<xsl:import href="https://raw.githubusercontent.com/Nictiz/HL7-mappings/master/ada_2_fhir/zibs2017/payload/package-2.0.5.xsl"/>
...
<xsl:param name="adaReleaseFile" select="document('../../../../projects/mp-mp910/definitions/mp-mp910-ada-release.xml')"/>
<xsl:param name="transactionId">2.16.840.1.113883.2.4.3.11.60.20.77.4.95</xsl:param>
<xsl:param name="doc-ada-new" select="document('../../../../projects/mp-mp910/new/sturen_voorschrift.xml')"/>
<xsl:param name="dir-4-ada" select="'C:\SVN\art_decor\trunk\ada-data\ada_2_wiki\mp\9.1.0\doseervoorbeelden\ada_instance\'"/>
...
{{DISPLAYTITLE:Medicatieproces 9.1: Voorbeelden doseringen}}
...
<xsl:text>
== </xsl:text><xsl:value-of select="$headingString"/> == <xsl:variable name="ada-files" select="collection(concat('ada_instance/', ., '?select=*.xml'))"/>
...
<xsl:template match="adaxml/data/*" mode="wiki"><!-- Override of the template from the imported stylesheet. Makes it a suitable for doseervoorbeelden -->
