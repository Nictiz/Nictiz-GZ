...
<xsl:stylesheet version="3.0" exclude-result-prefixes="#all" expand-text="false" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:array="http://www.w3.org/2005/xpath-functions/array" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:map="http://www.w3.org/2005/xpath-functions/map" xmlns:local="#local.2023040508344707303440200">
    ...
    <!-- == ADA Collection in original code: "ada_instance_MVE/?select=*.xml" == -->
...
{{DISPLAYTITLE:Sturen medicatiegegevens 9 2.0 - MVE: addendum inhoudelijke gegevens </xsl:text>
                                ...
                                <xsl:for-each-group select="current-group()" group-by="scenario-nr/@value">
                                    ...
                                    <rij>
                                        ...
                                        <xsl:if test="$mainScenario ne '0'">
                                            ...
                                            <kolom type="MVE id" waarde="{string-join(medicamenteuze_behandeling/(medicatieverstrekking|verstrekking)/identificatie/@value, '&lt;br/&gt;')}"/>
                                        </xsl:if>
                                    </rij>
                                </xsl:for-each-group>
                            </tabel-simpel>
...
==== </xsl:text>
    ...
    <xsl:template match="omschrijving[@conceptId = '2.16.840.1.113883.2.4.3.11.60.20.77.2.3.23241']" mode="maak-tabel-rij">
        <!-- Make table row 'gegevenselement' for omschrijving, override the default one, because we don't have a date config for this stuff -->
