...
<rule abstract="true"
   ...
   <assert role="error"
           ...
           see="https://art-decor.org/mediawiki/index.php?title=DTr1_URL">dtr1-2-URL: @value must be a valid URI, e.g. '
<value-of select="iri-to-uri(@value)"/>'.</assert>
   <!--
   ...
   <assert role="error"
           ...
           see="https://art-decor.org/mediawiki/index.php?title=DTr1_URL">dtr1-4-URL: [RIM-001] 
<value-of select="$name"/> SHALL NOT have nullFlavor, if there are other 
<value-of select="$name"/> elements which are not null</assert>
   <assert role="error"
           test="not($set_other[@use=$use][@value=$value])"
           see="https://art-decor.org/mediawiki/index.php?title=DTr1_URL">dtr1-5-URL: [RIM-002] 
<value-of select="$name"/> (
<value-of select="$value"/>) SHALL be distinct</assert>
