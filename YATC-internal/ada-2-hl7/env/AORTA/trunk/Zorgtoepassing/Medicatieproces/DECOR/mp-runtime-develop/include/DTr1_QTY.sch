...
<rule abstract="true"
   ...
   <assert role="error"
           ...
           see="https://art-decor.org/mediawiki/index.php?title=DTr1_QTY">dtr1-3-QTY: [RIM-001] 
...
<value-of select="$name"/> elements which are not null</assert>
   ...
   <assert role="error"
           ...
           see="https://art-decor.org/mediawiki/index.php?title=DTr1_QTY">dtr1-4-QTY: [RIM-002] 
<value-of select="$name"/> (@value=
<value-of select="$value"/> @unit=
<value-of select="$unit"/>) SHALL be distinct</assert>
</rule>