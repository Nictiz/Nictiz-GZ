...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="urn:hl7-org:v3" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:local="#local.2024011810474717992140100" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:hl7="urn:hl7-org:v3" xmlns:util="urn:hl7:utilities" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:nf="http://www.nictiz.nl/functions" xmlns:hl7nl="urn:hl7-nl:v3" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:pharm="urn:ihe:pharm:medication">
    <!-- ================================================================== -->
    <!--
        TBD
    -->
    ...
    <xsl:import href="../../../../ada-2-fhir/env/zibs2017/payload/ext-zib-medication-repeat-period-cyclical-schedule-2.0.xsl"/>
    ...
    <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9324_20201015132016" match="medicatieafspraak | medication_agreement">
        <!-- Mapping of medicatieafspraak concept in ADA ('eigen of ongedefinieerde MA') to HL7 CDA template 2.16.840.1.113883.2.4.3.11.60.20.77.10.9323 -->
        <xsl:param name="in" select=".">
    ...
    <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9325_20201515132423">
        <!-- Mapping of zib nl.zorg.Medicatieafspraak concept in ADA ('andermans MA') to HL7 CDA template 2.16.840.1.113883.2.4.3.11.60.20.77.10.9323 -->
        <xsl:param name="in" select=".">
            <!-- ADA Node to consider in the creation of the hl7 element -->
        </xsl:param>
        <xsl:for-each select="$in">
    ...
    <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9327_20201015133041" match="toedieningsafspraak" mode="HandleTAKopie920">
        <!-- Toedieningsafspraak vanaf 9.2.0 - kopie -->
        <xsl:param name="in" select=".">
    ...
    <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9348_20210105000000" match="toedieningsduur" mode="HandleCDAToedieningsduur9348">
        <!-- HL7NL PIVL_TS Toedieningsduur. Creates contents of a PIVL_TS element. Assumed context is toedieningsduur -->
        <xsl:param name="in" as="element(toedieningsduur)?" select=".">
            <!-- Input node (should be one ada element toedieningsduur). Defaults to context element. -->
        </xsl:param>
        <xsl:param name="operator" as="xs:string?" select="'I'">
            <!-- HL7 operator, optional. Defaults to I. -->
        </xsl:param>
        ...
        <xsl:for-each select="$in">
            ...
            <hl7nl:phase>
                <hl7nl:width>
                    <xsl:attribute name="xsi:type" select="'hl7nl:PQ'"/>
    ...
    <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9350_20210105175614" match="weekdag" mode="PIVLTSWeekdag9350">
        ...
        <xsl:param name="weekdagCode" select="$in/@code">
            <!-- The ada code for week day, which is in SNOMED. Defaults to $in element @code attribute -->
        </xsl:param>
    ...
    <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9351-9354" match="dagdeel" mode="HandleDagdeel9351-9354">
        <!-- HL7NL PIVL_TS Dagdeel. The HL7 templates are 9351 - 9354, but we make one xslt template here, because handling is mostly the same. -->
        <xsl:param name="in" as="element()?" select=".">
            <!-- The ada input dagdeel element. Defaults to context. -->
        </xsl:param>
        <xsl:param name="operator" as="xs:string?">
            <!-- Operator for the PIVL_TS. Optional, defaults to 'A' (intersect) -->
        </xsl:param>
    ...
    <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9349_202010105160422" match="toedieningsschema" mode="HandleCDAAdministrationSchedule9x">
        <!-- Create an MP CDA administration schedule based on ada toedieningsschema. Version 9.x -->
        <xsl:param name="in" as="element()*" select=".">
        ...
        <xsl:for-each select="$in">
            ...
            <xsl:if test="count(./*[name() ne 'is_flexibel'][.//(@value | @code | @unit)] | ../toedieningsduur[@value | @unit] | ../../../herhaalperiode_cyclisch_schema[@value | @unit]) gt 1">
                <effectiveTime operator="A">
                    <xsl:attribute name="xsi:type" select="'SXPR_TS'"/>
                    <!-- week day -->
                    <xsl:for-each select="weekdag[@code]">
                        <comp>
                            <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9350_20210105175614">
                                <xsl:with-param name="operator" select="                                         if (position() eq 1) then                                             'A'                                         else                                             'I'"/>
                            </xsl:call-template>
                        </comp>
                    </xsl:for-each>
                    <!-- part of day -->
    ...
    <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9383_20210618000000" match="contactpersoon" mode="handleAuthorContactpersoon92">
        <!--  MP CDA Author Participation for Contactpersoon  -->
    ...
    <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9385_20210628143123">
        ...
        <observation classCode="OBS" moodCode="EVN">
            ...
            <value>
                <xsl:attribute name="xsi:type" select="'CE'"/>
                <xsl:call-template name="makeCodeAttribs"/>
            </value>
        </observation>
    </xsl:template>
    ...
    <xsl:function name="nf:frequency-calculate">
        <!-- Temp function for super duper friendly frequency calculations, probably should not do this, but simply output the frequency when the input is shaky. Saving it before deleting -->
