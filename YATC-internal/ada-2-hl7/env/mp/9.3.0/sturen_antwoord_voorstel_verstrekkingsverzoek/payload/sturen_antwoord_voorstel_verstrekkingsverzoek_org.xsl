...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="urn:hl7-org:v3" xmlns:hl7="urn:hl7-org:v3" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:nf="http://www.nictiz.nl/functions" xmlns:hl7nl="urn:hl7-nl:v3" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:local="#local.2024011810474758233140100" xmlns:pharm="urn:ihe:pharm:medication" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    <!-- ================================================================== -->
    <!--
        TBD
    -->
    ...
    <xsl:template match="/">
        <xsl:call-template name="SturenAntwoordVoorstelVV-930">
            <xsl:with-param name="in" select="adaxml/data/sturen_antwoord_voorstel_verstrekkingsverzoek"/>
        </xsl:call-template>
    </xsl:template>

    <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->

    <xsl:template name="SturenAntwoordVoorstelVV-930">
        <xsl:param name="in" select="adaxml/data/sturen_antwoord_voorstel_verstrekkingsverzoek"/>
        ...
        <xsl:if test="$schematronRef">
            ...
            <xsl:processing-instruction name="xml-model">phase="#ALL" href="../../schematron_closed_warnings/mp-mp92_avvv.sch" type="application/xml" schematypens="http://purl.oclc.org/dsdl/schematron" phase="#ALL"</xsl:processing-instruction>
        </xsl:if>
        ...
        <organizer classCode="CLUSTER" moodCode="EVN" xmlns:cda="urn:hl7-org:v3">
            ...
            <templateId root="2.16.840.1.113883.2.4.3.11.60.20.77.10.9211"/>
            <xsl:for-each select="$in/voorstel_gegevens/antwoord/identificatie[@value | @root]">
                <xsl:call-template name="makeIIValue">
                    <xsl:with-param name="elemName">id</xsl:with-param>
                </xsl:call-template>
            </xsl:for-each>
            <code code="158" displayName="Sturen antwoord voorstel verstrekkingsverzoek" codeSystem="2.16.840.1.113883.2.4.3.11.60.20.77.4" codeSystemName="ART DECOR transacties"/>
            ...
            <xsl:for-each select="$in/voorstel_gegevens/antwoord[.//(@value | @code | @nullFlavor)]">
                <component typeCode="COMP">
                    <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9212_20180420174745"/>
                </component>
            </xsl:for-each>
        </organizer>
    </xsl:template>
