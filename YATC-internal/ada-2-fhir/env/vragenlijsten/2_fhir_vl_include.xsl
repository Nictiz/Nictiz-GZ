...
<xsl:stylesheet version="2.0" exclude-result-prefixes="#all" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:naf="http://www.nictiz.nl/ada-functions" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:uuid="http://www.uuid.org" xmlns:local="urn:fhir:stu3:functions">
    <!-- ================================================================== -->
    <!--
        TBD
    -->
    ...
    <xsl:template name="vl-vragenlijst-1.0.0">
        <!--  Template based on FHIR Profile http://nictiz.nl/fhir/StructureDefinition/nl-core-questionnaire -->
        <xsl:param name="vragenlijst" as="element()?" select=".">
            <!-- ada xml element vragenlijst -->
        </xsl:param>
        <xsl:param name="vragenlijst-id" as="xs:string?">
            <!-- optional technical id for the FHIR Task resource -->
        </xsl:param>
    ...
    <xsl:template name="vl-vragenlijst-antwoord-1.0.0">
        <!--  Template based on FHIR Profile http://nictiz.nl/fhir/StructureDefinition/nl-core-questionnaireresponse -->
        <xsl:param name="vragenlijst-antwoord" as="element()?" select=".">
            <!-- ada xml element vragenlijst-antwoord -->
        </xsl:param>
    ...
    <xsl:template name="QRReference" match="questionnaire_response" mode="doQRReference-1.0" as="element()*">
        ...
        <xsl:param name="in" as="element()?" select=".">
            <!-- input ada questionnare_response, defaults to context -->
        </xsl:param>
        <xsl:for-each select="$in">
            ...
            <xsl:choose>
                <xsl:when test="$theGroupElement">
                    <reference value="{nf:getFullUrlOrId($theGroupElement/f:entry)}" xmlns="http://hl7.org/fhir"/>
                </xsl:when>
            </xsl:choose>

            <xsl:if test="string-length($theGroupElement/reference-display) gt 0">
                <display value="{$theGroupElement/reference-display}" xmlns="http://hl7.org/fhir"/>
            </xsl:if>
        </xsl:for-each>
    </xsl:template>
    ...
    <xsl:template name="makeQuestionnaireRef">
        <!-- Makes a questionnaire reference attempting to find questionnaire -->
        <xsl:param name="in" select=".">
            <!-- questionnaire ada element containing url -->
        </xsl:param>
        ...
        <xsl:for-each select="$in">
            ...
            <reference value="{$inputUrl}" xmlns="http://hl7.org/fhir"/>
            <display value="{$adaQuestionnaire/title/value/@value}" xmlns="http://hl7.org/fhir"/>
        </xsl:for-each>
    </xsl:template>
    ...
    <xsl:template name="vl-vragenlijst-item-1.0.0" match="item | questionnaire_item" mode="doQuestItem-1.0.0">
        <!-- Helper template to create vragenlijst item, context should be ada vragenlijst.item element -->
    ...
    <xsl:template name="vl-antwoord-item-1.0.0" match="questionnaire_response_item" mode="doAnswerItem-1.0.0">
        <!-- Helper template to create questionnaire response item, context should be ada questionnaireresponse.item element -->
        <xsl:param name="in" select="." as="element()?">
            <!-- The element to be handled. Defaults to context element. -->
        </xsl:param>
    ...
    <xsl:template name="vl-vragenlijst-verwijzing-1.0.0">
        <!--  Template based on FHIR Profile http://nictiz.nl/fhir/StructureDefinition/QuestionnaireReferenceTask -->
        <xsl:param name="vragenlijstverwijzing" as="element()?">
            <!-- ada xml element vragenlijstverwijzing -->
        </xsl:param>
    ...
    <xsl:template name="vl-extension-renderingstyle-1.0.0" match="*" mode="doVragenlijstExtensionRenderingstyle-1.0.0">
        <!-- Helper template to create extension for rendering style, context is ada element with @value -->
        <extension url="http://hl7.org/fhir/StructureDefinition/rendering-style" xmlns="http://hl7.org/fhir">
            <valueString value="{@value}"/>
        </extension>
    </xsl:template>
    ...
    <xsl:template name="vl-extension-renderingxhtml-1.0.0" match="*" mode="doVragenlijstExtensionRenderingxhtml-1.0.0">
        <!-- Helper template to create extension for rendering xhtml, context is ada element with @value -->
        <extension url="http://hl7.org/fhir/StructureDefinition/rendering-xhtml" xmlns="http://hl7.org/fhir">
            <valueString value="{@value}"/>
        </extension>
    </xsl:template>
    ...
    <xsl:template name="vl-min-max-1.0.0" match="*[*]" mode="doVragenlijstMinMax-1.0.0">
        <!-- Helper template to make minValue maxValue extensions for Questionnaire item. Context should be an ada element with the appropriate subelements (datum(tijd), integer, decimaal getal, etc.) -->
        <xsl:for-each select="datum[@value]">
            <valueDate xmlns="http://hl7.org/fhir">
                <xsl:attribute name="value">
                    <xsl:call-template name="format2FHIRDate">
                        <xsl:with-param name="dateTime" select="@value"/>
                        <xsl:with-param name="dateT" select="$dateT"/>
        ...
        <xsl:for-each select="datum_tijd[@value]">
            <valueDateTime xmlns="http://hl7.org/fhir">
                <xsl:attribute name="value">
                    <xsl:call-template name="format2FHIRDate">
                        <xsl:with-param name="dateTime" select="@value"/>
                        <xsl:with-param name="dateT" select="$dateT"/>
        ...
        <xsl:for-each select="tijd[@value]">
            <valueTime xmlns="http://hl7.org/fhir">
                <xsl:attribute name="value">
                    <xsl:call-template name="format2FHIRDate">
                        <xsl:with-param name="dateTime" select="@value"/>
                        <xsl:with-param name="dateT" select="$dateT"/>
        ...
        <xsl:for-each select="moment[@value]">
            <valueInstant xmlns="http://hl7.org/fhir">
                <xsl:attribute name="value">
                    <xsl:call-template name="format2FHIRDate">
                        <xsl:with-param name="dateTime" select="@value"/>
                        <xsl:with-param name="dateT" select="$dateT"/>
        ...
        <xsl:for-each select="integer[@value]">
            <valueInteger value="{@value}" xmlns="http://hl7.org/fhir"/>
        </xsl:for-each>
        <xsl:for-each select="decimaal_getal[@value]">
            <valueDecimal value="{@value}" xmlns="http://hl7.org/fhir"/>
        </xsl:for-each>
    </xsl:template>
    ...
    <xsl:template name="valuesetReference" match="options" mode="doValueSetReference">
        <!-- Helper template to create FHIR valueSet reference, context should be ada options element -->
        <reference value="{nf:getFullUrlOrId('ValueSet',nf:getGroupingKeyDefault(.), false())}" xmlns="http://hl7.org/fhir"/>
        <xsl:for-each select="(weergavetekst | weergave_tekst)[@value]">
            <display value="{normalize-space(@value)}" xmlns="http://hl7.org/fhir"/>
        </xsl:for-each>
    </xsl:template>
    ...
    <xsl:function name="nf:getFullUrlOrId" as="xs:string?">
        ...
        <xsl:param name="resourceType" as="xs:string?">
            <!-- The type of resource to find, using hte variable with common entries -->
        </xsl:param>
    ...
    <xsl:function name="nf:get-specific-productcode" as="element()?">
        ...
        <xsl:sequence select="                 if ($ada-product-code[@codeSystem = $oidGStandaardZInummer]) then                     ($ada-product-code[@codeSystem = $oidGStandaardZInummer])                 else                     (if ($ada-product-code[@codeSystem = $oidGStandaardHPK]) then                         ($ada-product-code[@codeSystem = $oidGStandaardHPK])                     else                         (if ($ada-product-code[@codeSystem = $oidGStandaardPRK]) then                             ($ada-product-code[@codeSystem = $oidGStandaardPRK])                         else                             (if ($ada-product-code[@codeSystem = $oidGStandaardGPK]) then                                 ($ada-product-code[@codeSystem = $oidGStandaardGPK])                             else                                 $ada-product-code[1]                             )                         )                     )"/>
    </xsl:function>

    <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->

    <xsl:function name="nf:get-qr-display" as="xs:string?">
        <!-- Returns display for questionnaire response -->
        <xsl:param name="in" as="element()?">
            <!-- the ada questionnaire_response element to create a display for -->
        </xsl:param>
    ...
    <xsl:template name="valueSetEntry" match="options" mode="doValueSetEntry">
        <!-- Creates entry for ValueSet using options in questionnaire -->
        <entry xmlns="http://hl7.org/fhir">
            <fullUrl value="{nf:get-fhir-uuid(.)}"/>
            <resource>
    ...
    <xsl:template name="vragenlijstReference" match="vragenlijst | allowed_profile | antwoord_verwijzing | verwijzing" mode="doVragenlijstValueReference-1.0.0">
        <!-- Creates entry for ValueSet using options in questionnaire -->
        <reference value="{verwijzing/@value}" xmlns="http://hl7.org/fhir"/>
        <xsl:if test="weergave_tekst[string-length(@value) gt 0]">
            <display value="{weergave_tekst[string-length(@value) gt 0]/@value}" xmlns="http://hl7.org/fhir"/>
        </xsl:if>
    </xsl:template>
</xsl:stylesheet>
