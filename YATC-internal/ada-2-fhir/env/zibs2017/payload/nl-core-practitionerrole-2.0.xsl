...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://hl7.org/fhir" xmlns:naf="http://www.nictiz.nl/ada-functions" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:uuid="http://www.uuid.org" xmlns:local="urn:fhir:stu3:functions" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    <!-- ================================================================== -->
    <!--
        TBD
    -->
    ...
    <xsl:template match="@* | node()" mode="copy4PractitionerRoleKey">
        ...
        <xsl:copy>
            <xsl:apply-templates select="@* | node()" mode="copy4PractitionerRoleKey"/>
        </xsl:copy>
    </xsl:template>

    <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->

    <xsl:template match="zorgverlener_rol | health_professional_role" mode="copy4PractitionerRoleKey"><!-- Do not copy the role as it is not in FHIR PractitionerRole resource, so should not be part of duplicate detection --></xsl:template>
</xsl:stylesheet>
