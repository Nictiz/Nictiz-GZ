...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://hl7.org/fhir" xmlns:util="urn:hl7:utilities" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:uuid="http://www.uuid.org" xmlns:local="urn:fhir:stu3:functions" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    <!-- ================================================================== -->
    <!--
        TBD
    -->
    ...
    <xsl:template name="zib-Alert" as="element()">
        <!-- Template based on FHIR Profile http://nictiz.nl/fhir/StructureDefinition/zib-Alert  -->
        <xsl:variable name="resource">
            <Flag>
                <xsl:choose>
                    <xsl:when test="$referById">
    ...
    <xsl:template name="gp-Encounter" as="element()">
        <!-- Template based on FHIR Profile http://nictiz.nl/fhir/StructureDefinition/gp-Encounter. NOTE: this template is preliminary, some things are missing  -->
        <xsl:param name="encounterGroup" as="element()+">
            <!-- Some systems send as many copies of an encounter as there as episode links. We need all links. -->
        </xsl:param>
        <xsl:variable name="resource">
            <Encounter>
                <xsl:choose>
    ...
    <xsl:template name="zib-LaboratoryTestResult-4.1">
        <!--  Template based on FHIR Profile http://nictiz.nl/fhir/StructureDefinition/zib-LaboratoryTestResult-Observation  -->
    ...
    <xsl:template name="gp-DiagnosticResult">
        <!--  Template based on FHIR Profile http://nictiz.nl/fhir/StructureDefinition/gp-DiagnosticResult  -->
    ...
    <xsl:template name="gp-JournalEntry" match="journal_entry" mode="gp-JournalEntry">
        <!-- Template to facilitate mapping journal entries to the gp-JournalEntry FHIR instance (http://nictiz.nl/fhir/StructureDefinition/gp-JournalEntry, profile on Observation) -->
        <xsl:param name="author" as="element()*">
            <!-- Author entry from the bundle -->
        </xsl:param>
        <xsl:param name="subject" as="element()*">
            <!-- Subject entry from the bundle -->
        </xsl:param>
        <xsl:variable name="resource">
            <Observation>
                <xsl:choose>
    ...
    <xsl:template name="gp-EncounterReport">
        <!-- Template for constructing a gp-EncounterReport instance (http://nictiz.nl/fhir/StructureDefinition/gp-EncounterReport, profile on Composition) from a collection of gp-JournalEntry instances. -->
        <xsl:param name="author" as="element()*">
            <!-- Author entry from the bundle -->
        </xsl:param>
        <xsl:param name="subject" select="../bundle/subject/patient" as="element()*">
            <!-- Patient entry from the bundle -->
        </xsl:param>
        <xsl:param name="encounterreport-id" as="xs:string?"/>
        <xsl:param name="gp-JournalEntries" as="element()*">
            <!-- The journal entries to include as gp-JournalEntry instances -->
        </xsl:param>
            ...
            <Composition>
                ...
                <xsl:choose>
                    <xsl:when test="$e_entry">
                        <xsl:variable name="theTitle" as="xs:string*">
                            <xsl:for-each-group select="$e_entry//f:valueCodeableConcept//f:display/@value" group-by="lower-case(.)">
    ...
    <xsl:template match="f:search/f:mode" mode="updateSearchMode">
        <xsl:param name="mode" select="'include'"/>
        <xsl:element name="mode" namespace="http://hl7.org/fhir">
            <xsl:attribute name="value" select="$mode"/>
        </xsl:element>
    </xsl:template>
