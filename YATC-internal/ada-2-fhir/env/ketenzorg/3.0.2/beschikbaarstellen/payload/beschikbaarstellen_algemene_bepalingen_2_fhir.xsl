...
<xsl:stylesheet version="2.0" exclude-result-prefixes="#all" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:util="urn:hl7:utilities" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:local="#local.2023100511155216820560200">
    <!-- ================================================================== -->
    <!--
        This XSL was created to facilitate mapping from ADA BundleOfGeneralMeasurements-transaction, 
        to HL7 FHIR STU3 profiles http://nictiz.nl/fhir/StructureDefinition/zib-GeneralMeasurement.
    -->
    <!-- ================================================================== -->
    <!--
        Copyright © Nictiz
    ...
    <xsl:param name="matchResource" select="('Observation')" as="xs:string*"/>

    <xsl:variable name="usecase">generalmeasurements</xsl:variable>
    ...
    <xsl:template match="/">
        <!-- Start conversion. Handle interaction specific stuff for "beschikbaarstellen algemene bepalingen". -->
        <xsl:call-template name="BundleOfGeneralMeasurements"/>
    </xsl:template>

    <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->

    <xsl:template name="BundleOfGeneralMeasurements">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <Bundle xsl:exclude-result-prefixes="#all" xsi:schemaLocation="http://hl7.org/fhir http://hl7.org/fhir/STU3/fhir-all.xsd" xmlns="http://hl7.org/fhir">
    ...
    <xsl:template match="f:resource/*" mode="doResourceInResultdoc">
        ...
        <xsl:variable name="filename" as="xs:string" select="concat($usecase, '-', $zib-name, '-', ancestor::f:entry/f:fullUrl/tokenize(@value, '[/:]')[last()], '.xml')"/>
