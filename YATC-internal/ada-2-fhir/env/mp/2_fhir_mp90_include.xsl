...
<xsl:stylesheet version="2.0" exclude-result-prefixes="#all" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:uuid="http://www.uuid.org" xmlns:local="urn:fhir:stu3:functions">
            ...
            <entry xmlns="http://hl7.org/fhir">
                ...
                <resource>
                    <xsl:call-template name="zib-MedicationUse-2.0">
                        ...
                        <xsl:with-param name="medicationstatement-id" select="                                 if ($referById) then                                     nf:removeSpecialCharacters(./identificatie/@value)                                 else                                     ()"/>
                    </xsl:call-template>
                </resource>
                <xsl:if test="string-length($searchMode) gt 0">
                    <search>
    ...
    <xsl:template name="MA-in-MedicationRequest-2.0">
        <!--  Template based on FHIR Profile https://simplifier.net/NictizSTU3-Zib2017/ZIB-AdministrationAgreement/  -->
        ...
        <xsl:for-each select="$medicatieafspraak">
            <xsl:variable name="resource">
                <MedicationRequest xsl:exclude-result-prefixes="#all" xmlns="http://hl7.org/fhir">
                    ...
                    <xsl:for-each select="./lichaamslengte[.//@value]">
                        <supportingInformation>
                            ...
                            <xsl:variable name="datum-string" select="                                 if (./lengte_datum_tijd/@value) then                                 format-dateTime(./lengte_datum_tijd/@value, '[D01] [MN,*-3], [Y0001] [H01]:[m01]')                                 else                                 'onbekend'"/>
                    ...
                    <xsl:for-each select="./lichaamsgewicht[.//@value]">
                        <supportingInformation>
                            ...
                            <xsl:variable name="datum-string" select="                                 if (./gewicht_datum_tijd/@value) then                                 format-dateTime(./gewicht_datum_tijd/@value, '[D01] [MN,*-3], [Y0001] [H01]:[m01]')                                 else                                 'onbekend'"/>
    ...
    <xsl:template name="patient-612-1.0">
        <!-- Template for FHIR profile 612-patient-1.0 -->
    ...
    <xsl:template name="patient-entry-612" match="patient" mode="doPatientEntry612">
        ...
        <xsl:param name="searchMode" as="xs:string">
            <!--  --> include</xsl:param>
    ...
    <xsl:template name="zib-AdministrationAgreement-2.0">
        ...
        <xsl:for-each select="$toedieningsafspraak">
            <xsl:variable name="resource">
                <xsl:variable name="profileValue">http://nictiz.nl/fhir/StructureDefinition/zib-AdministrationAgreement</xsl:variable>
