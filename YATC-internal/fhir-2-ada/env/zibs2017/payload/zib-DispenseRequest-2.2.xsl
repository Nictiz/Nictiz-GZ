...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:local="urn:fhir:stu3:functions">
    ...
    <xsl:template match="f:MedicationRequest" mode="zib-DispenseRequest-2.2">
        <!-- Template to convert f:MedicationRequest to ADA verstrekkingsverzoek -->
    ...
    <xsl:template match="f:authoredOn" mode="zib-DispenseRequest-2.2">
        <!-- Template to convert f:authoredOn to datum -->
        <datum>
    ...
    <xsl:template match="f:agent" mode="zib-DispenseRequest-2.2">
        <!-- Template to convert f:agent to auteur -->
        <auteur>
            <xsl:choose>
    ...
    <xsl:template match="f:medicationReference" mode="zib-DispenseRequest-2.2">
        <!-- Template to convert f:medicationReference to te_verstrekken_geneesmiddel -->
        <xsl:variable name="referenceValue" select="f:reference/@value"/>
        <te_verstrekken_geneesmiddel>
    ...
    <xsl:template match="f:validityPeriod" mode="zib-DispenseRequest-2.2">
        <!-- Template to convert f:validityPeriod to verbruksperiode -->
    ...
    <xsl:template match="f:expectedSupplyDuration" mode="zib-DispenseRequest-2.2">
        <!-- Template to convert f:expectedSupplyDuration to duur -->
    ...
    <xsl:template match="f:performer" mode="zib-DispenseRequest-2.2">
        <!-- Template to convert f:performer to beoogd_verstrekker -->
        <beoogd_verstrekker>
            <xsl:choose>
                <xsl:when test="f:extension[@url = $practitionerrole-reference]">
    ...
    <xsl:template match="f:extension[@url = $zib-Dispense-Location]" mode="zib-DispenseRequest-2.2">
        <!-- Template to convert f:extension zib-Dispense-Location to afleverlocatie -->
    ...
    <xsl:template match="f:extension[@url = $zib-DispenseRequest-RelatedMedicationAgreement]" mode="zib-DispenseRequest-2.2">
        <!-- Template to convert f:extension zib-DispenseRequest-RelatedMedicationAgreement to relatie_naar_medicatieafspraak -->
        <relatie_naar_medicatieafspraak>
            <xsl:call-template name="Identifier-to-identificatie">
