...
<xsl:stylesheet version="3.0" exclude-result-prefixes="#all" expand-text="false" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:hl7="urn:hl7-org:v3" xmlns:narf="http://www.nictiz.nl/adarefs-functions" xmlns:array="http://www.w3.org/2005/xpath-functions/array" xmlns:nf="http://www.nictiz.nl/functions" xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:map="http://www.w3.org/2005/xpath-functions/map" xmlns:local="#local.2023032108485567995730100" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    ...
    <xsl:template match="/">
        <!-- Template to start conversion. 
            Outputs ada instance sturen_medicatievoorschrift in debug directory when $debug is true
    ...
    <xsl:template match="adaxml" mode="copy-for-resolve">
        <!-- 
         ...
         -->
        <xsl:copy>
            <xsl:attribute name="xsi:noNamespaceSchemaLocation">../../sturen_medicatievoorschrift/ada_schemas/ada_sturen_medicatievoorschrift.xsd</xsl:attribute>
    ...
    <xsl:template match="verzoek_uitvoeren_voorschrift" mode="copy-for-resolve">
        <!-- 
         ...
         -->
        <sturen_medicatievoorschrift app="mp-mp910" shortName="sturen_medicatievoorschrift" formName="sturen_voorschrift" transactionRef="2.16.840.1.113883.2.4.3.11.60.20.77.4.95" transactionEffectiveDate="2015-12-01T10:32:15" versionDate="" prefix="mp-" language="nl-NL" title="{@title}" desc="{@desc}" id="{concat($concat2id,@id)}">
            <xsl:apply-templates select="node()" mode="copy-for-resolve"/>
        ...
        <xsl:if test="following-sibling::*">
            <xsl:variable name="mbhs" as="element()?">
                <mbhs>
                    ...
                    <xsl:for-each select="following-sibling::verstrekkingsverzoek">
