...
<xsl:stylesheet version="3.0" exclude-result-prefixes="#all" expand-text="false" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:narf="http://www.nictiz.nl/adarefs-functions" xmlns:array="http://www.w3.org/2005/xpath-functions/array" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:map="http://www.w3.org/2005/xpath-functions/map" xmlns:local="#local.2023032108485582071390100" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    ...
    <xsl:template name="ResolveSubscenario" match="subscenario" mode="copy-for-resolve">
        ...
        <xsl:for-each select="$in">
            ...
            <adaxml>
                <xsl:attribute name="xsi:noNamespaceSchemaLocation">../../../<xsl:value-of select="$versionDir"/>/beschikbaarstellen_medicatiegegevens/ada_schemas/ada_beschikbaarstellen_medicatiegegevens.xsd</xsl:attribute>
                <xsl:copy-of select="ancestor::adaxml/meta"/>
                <data>
                    <beschikbaarstellen_medicatiegegevens app="{ancestor::adaxml/data/*/@app}" shortName="beschikbaarstellen_medicatiegegevens" formName="medicatiegegevens" transactionRef="{$transactionRef}" transactionEffectiveDate="{$transactionEffectiveDate}" prefix="mp-" language="nl-NL" title="{ancestor::adaxml/data/*/@title}" desc="{beschrijving/@value}" id="{narf:handle-ada-id(concat($concat2id, ancestor::adaxml/data/*/@id, '-', $subNr))}">
                        <!-- additional @id handling -->
                        <xsl:apply-templates select="@id" mode="copy-for-resolve"/>
                        ...
                        <xsl:if test="descendant::medicamenteuze_behandeling">
                            ...
                            <xsl:variable name="distinctMbhReferenceValues" select="distinct-values($mbhs/medicamenteuze_behandeling//*[@datatype = 'reference']/replace(@value, '(.*)#(.*)', '$2'))" as="xs:string*"/>                            
                            <bouwstenen>
    ...
    <xsl:template name="ResolveForRaadplegen" match="adaxml/data/*" mode="copy-for-resolve-raadplegen">
                ...
                <data>
                    <beschikbaarstellen_medicatiegegevens app="{ancestor::adaxml/data/*/@app}" shortName="beschikbaarstellen_medicatiegegevens" formName="medicatiegegevens" transactionRef="{$transactionRef}" transactionEffectiveDate="{$transactionEffectiveDate}" prefix="mp-" language="nl-NL" title="{ancestor::adaxml/data/*/@title}" desc="{beschrijving/@value}" id="{concat($concat2id, ancestor::adaxml/data/*/@id)}">
                        ...
                        <xsl:if test="descendant::medicamenteuze_behandeling">
                            <xsl:variable name="mbhs" as="element()?">
                                <mbhs>
                                    <!-- only unique building blocks -->
                                    <xsl:for-each-group select="descendant::medicatieafspraak" group-by="medicatieafspraak-id/concat(@value, @root)">
                                    ...
                                    <xsl:for-each-group select="descendant::wisselend_doseerschema" group-by="wisselend_doseerschema-id/concat(@value, @root)">
                                        <xsl:variable name="bouwsteenId" select="current-group()/wisselend_doseerschema-id"/>
                                        <xsl:variable name="resolvedBouwsteen" select="($inputFulladaFiles/adaxml/data/*/medicamenteuze_behandeling/wisselend_doseerschema[identificatie[@value = $bouwsteenId/@value][@root = $bouwsteenId/@root]])[1]"/>
                                        <medicamenteuze_behandeling conceptId="2.16.840.1.113883.2.4.3.11.60.20.77.2.4.41">
                                            <!-- copy mbh id, , append with patient shortname -->
                                            <xsl:copy-of select="$resolvedBouwsteen/../identificatie"/>
                                            <!-- <xsl:comment>Medication agreement (medicatieafspraak) with id <xsl:value-of select="$ma-id/@value"/> found in instance with id: <xsl:value-of select="$resolved-ma/ancestor::data/*/@id"/> and title: id: <xsl:value-of select="$resolved-ma/ancestor::data/*/@title"/></xsl:comment>-->
                                            <xsl:apply-templates select="$resolvedBouwsteen" mode="copy-for-override">
                                                <xsl:with-param name="overrideElement" select="." as="element()"/>
                                            </xsl:apply-templates>
                                        </medicamenteuze_behandeling>
                                    </xsl:for-each-group>
                                    <xsl:for-each-group select="descendant::verstrekkingsverzoek" group-by="verstrekkingsverzoek-id/concat(@value, @root)">
                                        <xsl:variable name="vv-id" select="current-group()/verstrekkingsverzoek-id"/>
                                        <xsl:variable name="resolved-vv" select="($inputFulladaFiles/adaxml/data/*/medicamenteuze_behandeling/verstrekkingsverzoek[identificatie[@value = $vv-id/@value][@root = $vv-id/@root]])[1]"/>
                                        <medicamenteuze_behandeling conceptId="2.16.840.1.113883.2.4.3.11.60.20.77.2.4.41">
                                            <!-- copy mbh id -->
                                            <xsl:copy-of select="$resolved-vv/../identificatie"/>
                                            <xsl:apply-templates select="$resolved-vv" mode="copy-for-override">
                                    ...
                                    <xsl:for-each-group select="descendant::toedieningsafspraak" group-by="toedieningsafspraak-id/concat(@value, @root)">
                                        <xsl:variable name="bouwsteenId" select="current-group()/toedieningsafspraak-id"/>
                                        <xsl:variable name="resolvedBouwsteen" select="($inputFulladaFiles/adaxml/data/*/medicamenteuze_behandeling/toedieningsafspraak[identificatie[@value = $bouwsteenId/@value][@root = $bouwsteenId/@root]])[1]"/>
                                        <medicamenteuze_behandeling conceptId="2.16.840.1.113883.2.4.3.11.60.20.77.2.4.41">
                                            <!-- copy mbh id -->
                                            <xsl:copy-of select="$resolvedBouwsteen/../identificatie"/>
                                            <xsl:apply-templates select="$resolvedBouwsteen" mode="copy-for-override">
                                    ...
                                    <xsl:for-each-group select="descendant::medicatieverstrekking | descendant::verstrekking" group-by="(medicatieverstrekking-id | verstrekking-id)/concat(@value, @root)">
                                        <xsl:variable name="bouwsteenId" select="current-group()/(medicatieverstrekking-id | verstrekking-id)"/>
                                        <xsl:variable name="resolvedBouwsteen" select="($inputFulladaFiles/adaxml/data/*/medicamenteuze_behandeling/(medicatieverstrekking | verstrekking)[identificatie[@value = $bouwsteenId/@value][@root = $bouwsteenId/@root]])[1]"/>
                                        <medicamenteuze_behandeling conceptId="2.16.840.1.113883.2.4.3.11.60.20.77.2.4.41">
                                            <!-- copy mbh id -->
                                            <xsl:copy-of select="$resolvedBouwsteen/../identificatie"/>
                                            <xsl:apply-templates select="$resolvedBouwsteen" mode="copy-for-override">
                                    ...
                                    <xsl:for-each-group select="descendant::medicatiegebruik | descendant::medicatie_gebruik" group-by="(medicatiegebruik-id | medicatie_gebruik-id)/concat(@value, @root)">
                                        <xsl:variable name="bouwsteenId" select="current-group()/(medicatiegebruik-id | medicatie_gebruik-id)"/>
                                        <xsl:variable name="resolvedBouwsteen" select="($inputFulladaFiles/adaxml/data/*/medicamenteuze_behandeling/(medicatiegebruik | medicatie_gebruik)[identificatie[@value = $bouwsteenId/@value][@root = $bouwsteenId/@root]])[1]"/>
                                        <medicamenteuze_behandeling conceptId="2.16.840.1.113883.2.4.3.11.60.20.77.2.4.41">
                                            <!-- copy mbh id -->
                                            <xsl:copy-of select="$resolvedBouwsteen/../identificatie"/>
                                            <xsl:apply-templates select="$resolvedBouwsteen" mode="copy-for-override">
                                    ...
                                    <xsl:for-each-group select="descendant::medicatietoediening" group-by="medicatietoediening-id/concat(@value, @root)">
                                        <xsl:variable name="bouwsteenId" select="current-group()/medicatietoediening-id"/>
                                        <xsl:variable name="resolvedBouwsteen" select="($inputFulladaFiles/adaxml/data/*/medicamenteuze_behandeling/medicatietoediening[identificatie[@value = $bouwsteenId/@value][@root = $bouwsteenId/@root]])[1]"/>
                                        <medicamenteuze_behandeling conceptId="2.16.840.1.113883.2.4.3.11.60.20.77.2.4.41">
                                            <!-- copy mbh id -->
                                            <xsl:copy-of select="$resolvedBouwsteen/../identificatie"/>
                                            <xsl:apply-templates select="$resolvedBouwsteen" mode="copy-for-override">
