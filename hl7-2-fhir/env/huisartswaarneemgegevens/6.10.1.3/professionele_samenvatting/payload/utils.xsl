...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:hl7="urn:hl7-org:v3" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:hl7nl="urn:hl7-nl:v3" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:local="urn:fhir:stu3:functions" xmlns:util="urn:hl7:utilities" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    <!-- ================================================================== -->
    <!--
        TBD
    -->
    ...
    <xsl:import href="../../../../../../ada-2-fhir/env/fhir/2_fhir_fhir_include.xsl"/>
    ...
    <xsl:template match="*" mode="doOperationOutcome">
        <!-- Handle acknowledgementDetail as OperationOutcome -->
        <xsl:if test="$issues">
    ...
    <xsl:template match="hl7:effectiveTime" mode="DosageTiming">
        <!-- 
            These are Medication Process 9 and out of scope for now: http://decor.nictiz.nl/medicatieproces/mp-html-20170601T173502/tmp-2.16.840.1.113883.2.4.3.11.60.20.77.10.9149-2016-07-25T134340.html
            These are Medication Process 6 as in use on the LSP: http://decor.nictiz.nl/medicatieproces/mp-html-20170601T173502/tmp-2.16.840.1.113883.2.4.3.11.60.20.77.10.110-2013-05-21T000000.html
         -->
    ...
    <xsl:function name="local:getDateTime" as="xs:string?">
        <!-- Get an xs:dateTime of xs:date from an HL7 ISO 8601 string -->
    ...
    <xsl:function name="local:getUri" as="xs:string?">
        <!-- Get the FHIR System URI based on an input OID from HL7. xs:anyURI if possible, urn:oid:.. otherwise -->
    ...
    <xsl:function name="local:getUriFromId" as="xs:string?">
        <!-- Tries to create an as stable as possible URI from and hl7:id element. urn:oid:@root.@extension if @root and @extension is numeric. urn:uuid:@extension if extension is uuid. urn:uuid:@root if @root is uuid. nf:get-fhir-uuid($id[1]) otherwise. -->
        <xsl:param name="id" as="element()+"/>
        <xsl:param name="maskIdentifiers" as="xs:string*">
            <!-- Array of 0..* @root values that should be masked -->
        </xsl:param>
        <xsl:choose>
    ...
    <xsl:function name="local:getIdFromId" as="xs:string?">
        <!-- Tries to create an as stable as possible id from and hl7:id element. @root.@extension if @root and @extension is numeric. @extension if extension is uuid. @root if @root is uuid. nf:get-fhir-uuid($id[1]) otherwise. -->
        <xsl:param name="id" as="element()+"/>
        <xsl:param name="maskIdentifiers" as="xs:string*">
            <!-- Array of 0..* @root values that should be masked -->
        </xsl:param>
    ...
    <xsl:function name="local:getOidDisplayName" as="xs:string?">
        <!-- Get the displayName based on an input OID from HL7. string if possible, just oid otherwise -->
        <xsl:param name="oid" as="xs:string?"/>
        <xsl:choose>
    ...
    <xsl:function name="local:getCDDisplayName" as="xs:string">
        <!-- Get the displayName based on an input CD for HL7. string if possible, 'Weergave ontbreekt' otherwise -->
        <xsl:param name="in" as="element()"/>
    ...
    <xsl:function name="local:getNameForId" as="element(hl7:name)*">
        <!-- Finds hl7:name elements as sibling of hl7:id elements in a given context (search context). Not every id has a name to it. Usually for efficiency reasons the name is mentioned once. -->
        <xsl:param name="searchScope" as="element()*"/>
        <xsl:param name="theIDs" as="element()*">
            <!-- All ids for the same person/organization/entity -->
        </xsl:param>
    ...
    <xsl:function name="local:getSeverity">
        ...
        <xsl:param name="in" as="item()">
            <!-- acknowledgementDetail/@typeCode or acknowledgement/@typeCode -->
        </xsl:param>
    ...
    <xsl:template name="AD-to-Address" as="element()*">
        ...
        <xsl:for-each select="$in[not(@nullFlavor)]">
            <address xmlns="http://hl7.org/fhir">
                ...
                <xsl:if test="@use">
                    <xsl:choose>
                        <xsl:when test="tokenize(@use, '\s') = ('WP','DIR','PUB')">
                            <use value="work"/>
                        </xsl:when>
                        <xsl:when test="@use = ('H','HP','HV')">
                            <use value="home"/>
                        </xsl:when>
                        <xsl:when test="@use = 'TMP'">
                            <use value="temp"/>
                        </xsl:when>
                        <xsl:when test="@use = 'OLD'">
                            <use value="old"/>
                        </xsl:when>
                    </xsl:choose>
                    <xsl:choose>
                        <xsl:when test="tokenize(@use, '\s') = 'PST' and tokenize(@use, '\s') = 'PHYS'">
                            <type value="both"/>
                        </xsl:when>
                        <xsl:when test="tokenize(@use, '\s') = 'PST'">
                            <type value="pst"/>
                        </xsl:when>
                        <xsl:when test="tokenize(@use, '\s') = 'PHYS'">
                            <type value="phys"/>
                        </xsl:when>
                    </xsl:choose>
                </xsl:if>
            ...
            <name xmlns="http://hl7.org/fhir">
                ...
                <xsl:if test="hl7:prefix[tokenize(@qualifier,'\s') = 'VV'] | hl7:family">
                    <family value="{hl7:prefix[tokenize(@qualifier,'\s') = 'VV'] | hl7:family | hl7:delimiter}">
                        <xsl:for-each select="                             hl7:prefix[tokenize(@qualifier,'\s') = 'VV'] |                              hl7:prefix[tokenize(@qualifier,'\s') = 'VV'] |                              hl7:family[tokenize(@qualifier,'\s') = 'SP'] |                              hl7:family[tokenize(@qualifier,'\s') = 'BR']">
                            <xsl:choose>
    ...
    <xsl:template name="II-to-Identifier" as="element()*">
        <!-- Creates an identifier element from an II datatype input element. Masks the value if applicable -->
        <xsl:param name="in" as="element()?">
            <!-- V3 Datatype II element, e.g. hl7:id -->
        </xsl:param>
        <xsl:param name="maskIdentifiers" as="xs:string*">
            <!-- Array of 0..* @root values that should be masked -->
        </xsl:param>
