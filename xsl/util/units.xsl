...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ucum="http://unitsofmeasure.org/ucum-essence" xmlns:util="urn:hl7:utilities" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:local="#local.2024020614533906444350100">
    <!-- ================================================================== -->
    <!--
        TBD
    -->
    ...
    <xsl:function name="nf:convertGstdBasiseenheid2UCUM" as="xs:string*">
        ...
        <xsl:choose>
            <xsl:when test="$GstdBasiseenheid_code castable as xs:integer">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:call-template name="util:logMessage">
                            <xsl:with-param name="level" select="$logERROR"/>
                            <xsl:with-param name="msg">Unsupported G-standaard basiseenheid: "<xsl:value-of select="$GstdBasiseenheid_code"/>".</xsl:with-param>
                        </xsl:call-template>
            ...
            <xsl:otherwise>
                ...
                <xsl:call-template name="util:logMessage">
                    <xsl:with-param name="level" select="$logERROR"/>
                    <xsl:with-param name="msg">G-standaard code is not an integer. Unsupported G-standaard basiseenheid: "<xsl:value-of select="$GstdBasiseenheid_code"/>".</xsl:with-param>
                </xsl:call-template>
                <xsl:value-of select="$GstdBasiseenheid_code"/>
            </xsl:otherwise>
        </xsl:choose>
