...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:file="http://expath.org/ns/file" xmlns:util="urn:hl7:utilities" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:svrl="http://purl.oclc.org/dsdl/svrl" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:local="#local.2024020614533923287890100" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    <!-- ================================================================== -->
    <!--
        TBD
    -->
    ...
    <xsl:template match="/">
        ...
        <xsl:if test="$results//fail">
            ...
            <xsl:for-each select="$results//file[fail]">
                <xsl:call-template name="util:logMessage">
                    <xsl:with-param name="level" select="$logERROR"/>
                    <xsl:with-param name="msg"><xsl:text>

</xsl:text><xsl:value-of select="position()"/> FAILED: <xsl:value-of select="replace(resolve-uri(@name), '^file:', '')"/></xsl:with-param>
                </xsl:call-template>
            </xsl:for-each>
        </xsl:if>
